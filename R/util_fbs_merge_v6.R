...
#' v6dat_scored <- util_fbs_merge_v6(date_str = '2021-10-11')
#'
#' #if in same working directory as data and covid collecte data has different dates:
#' v6dat_scored <- util_fbs_merge_v6(child_date_str = '2021-10-11', child_home_date_str = '2021-9-15', child_lab_date_str = = '2021-9-15', parent_date_str = '2021-10-11')
#'
#' \dontrun{
#' #date must be a string. The following will not run:
#' v6dat_scored <- util_fbs_merge_v6(2021-10-11)
#'
#' #date must match the file name - for file named 'Child_V6_Home_2021_09_15', the
#' following will not run:
#' v6dat_scored <- util_fbs_merge_v6('2021_10_11')
#' }
#'
...
util_fbs_merge_v6 <- function(date_str, child_date_str, parent_date_str, data_path) {
    ...
    if (isTRUE(datestr_arg)) {
        child_v6dat <- util_fbs_child_v6dat(date_str, data_path)
        parent_v6dat <- util_fbs_parent_v6dat(date_str, data_path)
    } else {
        child_v6dat <- util_fbs_child_v6dat(child_date_str, data_path)
        parent_v6dat <- util_fbs_parent_v6dat(parent_date_str, data_path)
    }
