...
util_fbs_parent_v6dat <- function(file_pattern, data_path) {
    ...
    # Qualtrics data
    if (isTRUE(datapath_arg)) {
        qv6_parent_pathlist <- list.files(path = data_path, pattern = file_pattern, full.names = TRUE)
    } else {
        qv6_parent_pathlist <- paste0(pattern = file_pattern, full.names = TRUE)
    }

    # check number of files found
    if (length(qv6_parent_pathlist) > 1) {
    ...
    qv6_parent_clean <- merge(qv6_parent_clean, visit_dates[c('id', 'RO1_V6_Date')], by = 'id', all.x = TRUE, all.y = FALSE)

    #update start_date
    qv6_parent_clean[["start_date"]] <- ifelse(!is.na(qv6_parent_clean[['RO1_V6_Date']]), as.character(qv6_parent_clean[['RO1_V6_Date']]), as.character(qv6_parent_clean[["start_date"]]))

    #remove RO1_V date column
    qv6_parent_clean <- qv6_parent_clean[, names(qv6_parent_clean) != "RO1_V6_Date"]

    # add label
    qv6_parent_clean_labels[["start_date"]] <- "date from participant contacts databases ('verified_visit_dates*.csv) converted to format yyyy-mm-dd in R. If no date in database, uses start_date metadata from qualtrics"
