...
qualtrics_parent_v1dat <- function(date_str, data_path) {
    ...
    # child information (sex, dob,) 2) anthro - h/w, puberty, PA,
    # parent measured h/w, parent related info, 3) general
    # demographics, 4) meal related Q's - ranking, eat out, PSS,
    # feeding strategies, 6) Fasting procedure

    qv1_parent_clean <- qv1_parent_clean[c(2, 1, 127, 5:15, 128:201,
        16:20, 371:377, 213:218, 21:51, 125:126, 52:124, 206:212,
        202:205, 219:370, 3:4)]

    qv1_parent_clean_labels <- qv1_parent_clean_labels[c(2, 1, 127,
        5:15, 128:201, 16:20, 371:377, 213:218, 21:51, 125:126, 52:124,
    ...
    names(qv1_parent_clean) <- c("id", "start_date", "sex", "dob",
        ...
        "birth_order", "ethnicity", "race", "pds_1", "pds_2", "pds_3",
        "pds_4m", "pds_5m", "pds_6m", "pds_4f", "pds_5fa", "pds_5fb",
        "pds_5fc", "pds_5fd", "pds_6f", "tanner_male", "tanner_female",
        "pa_m_wakeup", "pa_m_travel_school", "pa_m_traveltime_school",
        ...
        "pa_f_bedtime", "parent_respondant", "parent_respondant_other",
        "parent_dob", "parent_ethnicity", "parent_race", "parent_height1",
        ...
        "fs_wantmoreinfo", "child_broke_fast", "child_intake_premeal")
    ...
    for (v in 1:length(parent_anthro_vars)) {
        ...
        if (is.element(99, qv1_parent_clean[[pvar]])) {
            pna_dat <- ifelse(is.na(qv1_parent_clean[[pvar]]), 0,
                ifelse(qv1_parent_clean[[pvar]] == 99, 1, 0))
    ...
    for (v in 1:length(pa_vars)) {
        ...
        if (grepl("wakeup", pvar, fixed = TRUE)) {
            ...
            # (values are kept in the categorical version of this variable)
            qv1_parent_clean[[pvar]] <- ifelse(is.na(qv1_parent_clean[[pvar]]) |
                qv1_parent_clean[[pvar]] == "After 10:00 PM" | qv1_parent_clean[[pvar]] ==
                "Don't want to answer", NA, as.character(qv1_parent_clean[[pvar]]))
    ...
    level99_issue_catvars <- names(qv1_parent_clean)[c(3, 8, 10,
        13:14, 89, 92:93, 123:124, 126:127, 129, 131:132, 134:135,
        137:140, 142, 144, 147:155, 157, 159:160, 203:212, 226, 232,
        238, 244, 250, 256, 262, 268, 274, 280, 286, 292, 298, 304,
        310, 316, 322, 328, 334, 340, 346, 352, 358)]
    ...
    for (v in 1:length(wkhr_vars)) {
        ...
        # variable)
        qv1_parent_clean[[pvar]] <- ifelse(is.na(qv1_parent_clean[[pvar]]) |
            qv1_parent_clean[[pvar]] == 99 | qv1_parent_clean[[pvar]] ==
            61, NA, as.numeric(qv1_parent_clean[[pvar]]))
    }

    ## birth order ####
    if (is.element(99, qv1_parent_clean$birth_order)) {
        pna_dat <- ifelse(is.na(qv1_parent_clean$birth_order), 0,
            ifelse(qv1_parent_clean$birth_order == 99, 1, 0))
        ...
        # add label to pna database
        qv1_parent_pna_labels[["birth_order_pna"]] <- paste0("prefer not to answer marked for variable ",
            pvar, ": ", qv1_parent_clean_labels[["birth_order"]])
    ...
    qv1_parent_clean$birth_order <- ifelse(is.na(qv1_parent_clean$birth_order) |
        qv1_parent_clean$birth_order == " Don't want to answer",
        NA, as.character(qv1_parent_clean$birth_order))

    # split components
    birth_split <- matrix(unlist(strsplit(qv1_parent_clean$birth_order,
        " of ")), ncol = 2, byrow = TRUE)

    # assign to 2 variables and make numeric
    qv1_parent_clean$birth_order <- birth_split[, 1]
    qv1_parent_clean$n_siblingsbirth <- as.numeric(birth_split[,
        2])

    # replace 'st', 'nd', 'rd', and 'th' in birth order
    qv1_parent_clean$birth_order <- as.numeric(gsub("st|nd|rd|th",
        "", qv1_parent_clean$birth_order))

    # update labels
    qv1_parent_clean_labels[["birth_order"]] <- paste0(qv1_parent_clean_labels[["birth_order"]],
        " - converted to numeric by R")
    ...
    time_vars <- names(qv1_parent_clean)[c(29, 32:33, 40:41, 44:45,
        52:53, 56:57, 64:65, 68:69, 76:77, 80:81, 88)]

    for (v in 1:length(time_vars)) {
        pvar <- time_vars[v]

        # convert time
        qv1_parent_clean[[pvar]] <- format(as.POSIXlt(qv1_parent_clean[[pvar]],
    ...
    relevel_vars <- names(qv1_parent_clean)[c(10, 30:31, 34:35, 42:43,
        46:47, 54:55, 58:59, 66:67, 70:71, 78:79, 82:83, 124, 126:127,
        129, 139:140, 142, 144, 159:160, 226, 232, 238, 244, 250,
        256, 262, 268, 274, 280, 286, 292, 298, 304, 310, 316, 322,
        328, 334, 340, 346, 352, 358, 380:399)]

    for (v in 1:length(relevel_vars)) {
        pvar <- relevel_vars[v]

        # subtract 1 from level values so start at 0 rather than 1
        attributes(qv1_parent_clean[[pvar]])$labels <- attributes(qv1_parent_clean[[pvar]])$labels -
            1
    ...
    qv1_parent_clean$sex <- sjlabelled::set_labels(qv1_parent_clean$sex,
    ...
    qv1_parent_clean_labels[["sex"]] <- paste0(qv1_parent_clean_labels[["sex"]],
        " re-leveled in R to start with 0")
    ...
    for (v in 1:length(race_vars)) {
        ...
        qv1_parent_clean[[pvar]] <- sjlabelled::set_labels(qv1_parent_clean[[pvar]],
            labels = c(`White/Caucasian` = 0, `American Indian/Alaskan Native` = 1,
                Asian = 2, `Black/African American` = 3, `Hawaiian/Pacific Islander` = 4))
        set_attr <- attributes(qv1_parent_clean[[pvar]])
        qv1_parent_clean[[pvar]] <- ifelse(is.na(qv1_parent_clean[[pvar]]),
            NA, ifelse(qv1_parent_clean[[pvar]] == 5, 0, qv1_parent_clean[[pvar]]))
        attributes(qv1_parent_clean[[pvar]]) <- set_attr
        qv1_parent_clean_labels[[pvar]] <- paste0(qv1_parent_clean_labels[[pvar]],
            " re-leveled in R to start with 0 and set 'White/Caucasion' = 0")
    }
    ...
    qv1_parent_clean[161:202] <- sapply(qv1_parent_clean[161:202],
    ...
    qv1_parent_clean <- qv1_parent_clean[c(1:12, 402, 13:28, 380,
        29:31, 381, 32, 382, 33:39, 383, 40, 384, 41:43, 385, 44,
        386, 45:51, 387, 52, 388, 53:55, 389, 56, 390, 57:63, 391,
        64, 392, 65:67, 393, 68, 394, 69:75, 395, 76, 396, 77:79,
        397, 80, 398, 81:87, 399, 88, 89:103, 378, 104:106, 379,
        403:408, 107:133, 400, 134:136, 401, 137:377)]

    qv1_parent_clean_labels <- qv1_parent_clean_labels[c(1:12, 402,
        13:28, 380, 29:31, 381, 32, 382, 33:39, 383, 40, 384, 41:43,
        385, 44, 386, 45:51, 387, 52, 388, 53:55, 389, 56, 390, 57:63,
        391, 64, 392, 65:67, 393, 68, 394, 69:75, 395, 76, 396, 77:79,
        397, 80, 398, 81:87, 399, 88, 89:103, 378, 104:106, 379,
        403:408, 107:133, 400, 134:136, 401, 137:377)]
    ...
    qv1_parent_pna[2:n_pna_cols] <- as.data.frame(lapply(qv1_parent_pna[2:n_pna_cols],
    ...
    qv1_parent_clean = sjlabelled::set_label(qv1_parent_clean, label = matrix(unlist(qv1_parent_clean_labels,
        use.names = FALSE)))
    qv1_parent_pna = sjlabelled::set_label(qv1_parent_pna, label = matrix(unlist(qv1_parent_pna_labels,
        use.names = FALSE)))

    # make list of data frame and associated labels
    qv1_parent <- list(data = qv1_parent_clean, dict = qv1_parent_clean_labels,
        pna_data = qv1_parent_pna, pna_dict = qv1_parent_pna_labels)
