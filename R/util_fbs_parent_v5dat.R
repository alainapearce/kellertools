...
util_fbs_parent_v5dat <- function(file_pattern, data_path) {
    ...
    # Qualtrics data
    if (isTRUE(datapath_arg)) {
        qv5_parent_pathlist <- list.files(path = data_path, pattern = file_pattern, full.names = TRUE)
    } else {
        qv5_parent_pathlist <- paste0(pattern = file_pattern, full.names = TRUE)
    }

    # check number of files found
    if (length(qv5_parent_pathlist) > 1) {
