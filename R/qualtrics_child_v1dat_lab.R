...
qualtrics_child_v1dat_lab <- function(date_str, data_path) {
    ...
    # 4) re-ordering and re-name data columns general order #### 1) child information (id, startdate, sex, dob,
    # h/w, bmi, screen out), 2) freddies, 3) food VAS 4) intakes (preMeal, EAH, meal duration), 5) wanting, PSD,
    # PSS, etc 6) notes
    ...
    names(qv1_child_clean) <- c("id", "start_date", "sex", "dob", "height1", "height2", "weight1", "weight2", "height_avg",
        ...
        "want_skittles", "want_trains", "want_trucks", "want_starbursts", "want_turkey_sndwch", "food_initials", "child_notes")
