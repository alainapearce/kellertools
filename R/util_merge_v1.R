#' util_merge_v1: Merge all Visit 1 raw Qualtrics databases and organize variable in database order
#'
...
util_merge_v1 <- function(date_str, child_date_str, child_home_date_str, child_lab_date_str, parent_date_str, data_path) {
    ...
    if (isTRUE(datestr_arg)) {
        child_v1dat <- util_child_v1dat(date_str, data_path)
        child_home_v1dat <- util_child_v1dat_home(date_str, data_path)
        child_lab_v1dat <- util_child_v1dat_lab(date_str, data_path)
        parent_v1dat <- util_parent_v1dat(date_str, data_path)
    } else {
        child_v1dat <- util_child_v1dat(child_date_str, data_path)
        child_home_v1dat <- util_child_v1dat_home(child_home_date_str, data_path)
        child_lab_v1dat <- util_child_v1dat_lab(child_lab_date_str, data_path)
        parent_v1dat <- util_parent_v1dat(parent_date_str, data_path)
    }
