...
#' v4dat_scored <- util_fbs_merge_v4(date_str = '2021-10-11')
#'
#' #if in same working directory as data and covid collecte data has different dates:
#' v4dat_scored <- util_fbs_merge_v4(child_date_str = '2021-10-11', child_home_date_str = '2021-9-15', child_lab_date_str = = '2021-9-15', parent_date_str = '2021-10-11')
#'
#' \dontrun{
#' #date must be a string. The following will not run:
#' v4dat_scored <- util_fbs_merge_v4(2021-10-11)
#'
#' #date must match the file name - for file named 'Child_V4_Home_2021_09_15', the
#' following will not run:
#' v4dat_scored <- util_fbs_merge_v4('2021_10_11')
#' }
#'
...
util_fbs_merge_v4 <- function(date_str, child_date_str, child_home_date_str, child_lab_date_str, parent_date_str, data_path) {
    ...
    if (isTRUE(datestr_arg)) {
        child_v4dat <- util_fbs_child_v4dat(date_str, data_path)
        child_home_v4dat <- util_fbs_child_v4dat_home(date_str, data_path)
        child_lab_v4dat <- util_fbs_child_v4dat_lab(date_str, data_path)
        parent_v4dat <- util_fbs_parent_v4dat(date_str, data_path)
    } else {
        child_v4dat <- util_fbs_child_v4dat(child_date_str, data_path)
        child_home_v4dat <- util_fbs_child_v4dat_home(child_home_date_str, data_path)
        child_lab_v4dat <- util_fbs_child_v4dat_lab(child_lab_date_str, data_path)
        parent_v4dat <- util_fbs_parent_v4dat(parent_date_str, data_path)
    }
