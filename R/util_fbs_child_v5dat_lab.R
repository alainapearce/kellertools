...
util_fbs_child_v5dat_lab <- function(file_pattern, data_path) {
    ...
    if (isTRUE(datapath_arg)) {
        ...
        qv5_child_path <- list.files(path = paste0(data_path, '/Final_Covid/'), pattern = paste0(file_pattern, '_Lab'), full.names = TRUE)

        #if no files found, check direct data_path entered
        if (length(qv5_child_path) == 0) {
            qv5_child_path <- list.files(path = data_path, pattern = paste0(file_pattern, '_Lab'), full.names = TRUE)
        }
    } else {
        qv5_child_path <- paste0(pattern = paste0(file_pattern, '_Lab'), full.names = TRUE)
    }
