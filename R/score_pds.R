...
#' @param parID (optional) name of participant ID column in pds_data. If included the output dataset will be matched by parID, if not included the output dataset will be in the order of pds_data but will have no participant identifier.
#'
...
score_pds <- function(pds_data, respondent, male = 0, female = 1, parID) {
        ...
        if (isTRUE(male_arg) | isTRUE(female_arg)){
            if (sum(isTRUE(male_arg), isTRUE(female_arg)) == nsex_unique) {
                if (nsex_unique == 2) {
                    ...
                    pds_data_edits[['sex']] <- ifelse(pds_data_edits[['sex']] == male, 0, 1)
                    pds_data_edits[['sex']] <- factor(pds_data_edits[['sex']], levels = c(0, 1))
    ...
    for (var in 1:length(pds_varnames)){
        ...
        if (var_name %in% names(pds_data_edits)){
            ...
            #recode menarche
            if (var_name == 'pds_5fa'){
                pds_data_edits[[var_name]] <- ifelse(is.na(pds_data_edits[[var_name]]), NA,
                                                     ifelse(pds_data_edits[[var_name]] == 1, 4, 0))
            }
        }
    }
    ...
    pds_data_edits[['pds_score']] <- ifelse(pds_data_edits[['sex']] == 0,
                                            ifelse(pds_data_edits[['pds_score_na']] <= 1, rowMeans(pds_data_edits[male_vars], na.rm = TRUE), NA),
                                            ifelse(pds_data_edits[['pds_score_na']] <= 1 & !is.na( pds_data_edits[['pds_5fa']]), rowMeans(pds_data_edits[female_vars], na.rm = TRUE), NA))
    ...
    pds_data_edits[['pds_tanner_cat']] <- ifelse(is.na(pds_data_edits[['pds_tanner_sum']]), NA,
        ifelse(pds_data_edits[['sex']] == 0,
               ifelse(pds_data_edits[['pds_tanner_sum']] == 12, 'Postpubertal',
                      ifelse(pds_data_edits[['pds_tanner_sum']] >=9 , 'Late Pubertal',
                             ifelse(pds_data_edits[['pds_tanner_sum']] >=6, ifelse(pds_data_edits['pds_2'] < 4 & pds_data_edits['pds_4m'] < 4 & pds_data_edits['pds_5m'] < 4, 'Mid-Pubertal', 'Late Pubertal'),
                                    ifelse(pds_data_edits[['pds_tanner_sum']] >=4, ifelse(pds_data_edits['pds_2'] < 3 & pds_data_edits['pds_4m'] < 3 & pds_data_edits['pds_5m'] < 3, 'Early Pubertal', 'Mid-Pubertal'), 'Prepubertal')))),
               ifelse(pds_data_edits[['pds_5fa']] == 4,
                      ifelse(pds_data_edits[['pds_tanner_sum']] == 8, 'Postpubertal',
                             ifelse(pds_data_edits[['pds_tanner_sum']] <= 7, 'Late Pubertal', NA)),
                      ifelse(pds_data_edits[['pds_tanner_sum']] > 3, 'Mid-Pubertal',
                             ifelse(pds_data_edits[['pds_tanner_sum']] == 3, 'Early Pubertal',
                                    ifelse(pds_data_edits[['pds_tanner_sum']] == 2, 'Prepubertal', NA))))))
    ...
    if (isTRUE(ID_arg)){
        pds_score_dat <- data.frame(id = pds_data_edits[[parID]],
                                    sex = pds_data_edits[['sex']],
                                    pds_score = pds_data_edits[['pds_score']],
                                    pds_tanner_cat = pds_data_edits[['pds_tanner_cat']])
    } else {
        pds_score_dat <- data.frame(sex = pds_data_edits[['sex']],
                                    pds_score = pds_data_edits[['pds_score']],
                                    pds_tanner_cat = pds_data_edits[['pds_tanner_cat']])
    }
