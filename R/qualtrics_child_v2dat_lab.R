...
qualtrics_child_v2dat_lab <- function(date_str, data_path) {
    ...
    if (isTRUE(datapath_arg)) {
        qv2_child_path <- paste0(data_path, "/Final_CovidAtHome/Child_V2_Lab_", date_str,
                                 ".sav")
    } else {
    ...
    names(qv2_child_clean) <- c("id", "start_date", "freddy_pre_meal",
    ...
    # 8) random fixes to factor level names and variable descriptions
    ## no levels or variables descriptions to fix
