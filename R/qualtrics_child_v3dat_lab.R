#' qualtrics_child_v3dat_lab: Process raw qualtrics visit 3 data for the child
#'
...
#' @export
#'
qualtrics_child_v3dat_lab <- function(date_str, data_path) {
    ...
    if (isTRUE(datapath_arg)) {
        qv3_child_path <- paste0(data_path, "/Final_CovidAtHome/Child_V3_Lab_", date_str,
                                 ".sav")
    } else {
    ...
    ## re-name variables
    names(qv3_child_clean) <- c("id", "start_date", "freddy_pre_meal",
