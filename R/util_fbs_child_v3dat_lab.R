...
util_fbs_child_v3dat_lab <- function(file_pattern, data_path) {
    ...
    if (isTRUE(datapath_arg)) {
        #check pattern of directories specified in Data manual
        qv3_child_pathlist <- list.files(path = paste0(data_path, '/Final_Covid/'), pattern = paste0(file_pattern, '_Lab'), full.names = TRUE)

        #if no files found, check direct data_path entered
        if (length(qv3_child_pathlist) == 0) {
            qv3_child_pathlist <- list.files(path = data_path, pattern = paste0(file_pattern, '_Lab'), full.names = TRUE)
        }
    } else {
        qv3_child_pathlist <- paste0(pattern = paste0(file_pattern, '_Lab'), full.names = TRUE)
    }
