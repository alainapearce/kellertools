...
qualtrics_parent_v2dat <- function(date_str, data_path) {
    ...
    # 4) CEBQ, CFQ, BES, FFB, 5) CBQ, 6) updates

    qv2_parent_clean <- qv2_parent_clean[c(2, 1, 3, 212:229, 111:211, 17:18, 80, 19:25,
                                           81, 26:29, 82, 30, 83:84, 31, 85, 32:34, 86,
                                           35:42, 87:89, 43:48, 90, 49:52, 91:93, 53:54,
                                           94:95, 55, 96, 56:58, 97:98, 59:64, 99,
                                           65:69, 100:101, 70:71, 102, 72, 103, 73,
                                           104:106, 74:78, 107:110, 79, 4:16)]

    qv2_parent_clean_labels <- qv2_parent_clean_labels[c(2, 1, 3, 212:229, 111:211, 17:18, 80, 19:25,
                                                         81, 26:29, 82, 30, 83:84, 31, 85, 32:34, 86,
                                                         35:42, 87:89, 43:48, 90, 49:52, 91:93, 53:54,
                                                         94:95, 55, 96, 56:58, 97:98, 59:64, 99,
                                                         65:69, 100:101, 70:71, 102, 72, 103, 73,
    ...
    for (v in 1:length(level99_issue_catvars)) {
        ...
        if (is.element(99, qv2_parent_clean[[pvar]])) {
            pna_dat <- ifelse(is.na(qv2_parent_clean[[pvar]]),
                              0, ifelse(qv2_parent_clean[[pvar]] == 99, 1, 0))

            if (length(names(qv2_parent_pna)) == 0 ){
                new_pna <- 1
                qv2_parent_pna <- data.frame(pna_dat)
            } else {
            ...
            qv2_parent_pna_labels[[paste0(pvar, "_pna")]] <- paste0("prefer not to answer marked for variable ", pvar, ": ", qv2_parent_clean_labels[[pvar]])
    ...
    for (var in 1:length(cshq_vars)){
        ...
        #reset all labels
        qv2_parent_clean[[var_name]] <- sjlabelled::set_labels(qv2_parent_clean[[var_name]], labels = c("Never" = 1, 'Rarely' = 2, "Sometimes" = 3, "Usually" = 4, "Always" = 5))

        #save attributes
        set_attr <- attributes(qv2_parent_clean[[var_name]])

        #re-set values to match new level values
        qv2_parent_clean[[var_name]] <- ifelse(is.na(qv2_parent_clean[[var_name]]), NA,
                                               ifelse(qv2_parent_clean[[var_name]] == 0, 1,
                                                      ifelse(qv2_parent_clean[[var_name]] == 1, 2,
                                                             ifelse(qv2_parent_clean[[var_name]] == 5, 4,
    ...
    ## 8c) FFB has wrong level values - should start at 0 - Never True to 4 - Always True
    ...
    ## 8d) CBQ - change NA option to be more clear and set to -99
    #CBQ states: "If you cannot answer one of the items because you have never seen
    #the child in that situation, for example, if the statement is about the child's
    #reaction to your singing and you have never sung to your child, then
    ...
    for (var in 1:length(cbq_vars)){
        ...
        #subtract 1 from each data value to match new level values
        qv2_parent_clean[[var_name]] <- ifelse(is.na(qv2_parent_clean[[var_name]]), NA,
                                               ifelse(qv2_parent_clean[[var_name]] == 99,  -99, qv2_parent_clean[[var_name]]))
