...
#' ch_v7_dat <- util_fbs_child_v7dat('2021-10-11')
#'
#' \dontrun{
#' #date must be a string. The following will not run:
#' ch_v7_dat <- util_fbs_child_v7dat(2021-10-11)
#'
#' #date must match the file name - for file named 'Child_V7_2021_09_16', the
#' following will not run:
#' ch_v7_dat <- util_fbs_child_v7dat('2021_10_11')
#' }
#'
#'
#' @export
#'
util_fbs_child_v7dat <- function(date_str, data_path) {
    ...
    if (isTRUE(datapath_arg)) {
        qv7_child_path <- paste0(data_path, "/", "Child_V7_", date_str, ".sav")
    } else {
        qv7_child_path <- paste0("Child_V7_", date_str, ".sav")
    }
