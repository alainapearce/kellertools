...
qualtrics_child_v4dat <- function(date_str, data_path) {
    ...
    if (isTRUE(datapath_arg)) {
        qv4_child_path <- paste0(data_path, "/", "Child_V4_", date_str,
            ".sav")
    } else {
    ...
    # meal duration), 5) CWC, CBIS, Parent Responsiveness, alien snack, mock 6) notes
    ...
    #NOTE - there is a whole row I am not manually fixing as every value has ',' instead of '.'
    for (var in 1:length(intake_vars)){
        var_name <- intake_vars[[var]]

        qv4_child_clean[[var_name]] <- ifelse(qv4_child_clean[[var_name]] == '-' | qv4_child_clean[[var_name]] == 'NA' , NA, qv4_child_clean[[var_name]])
