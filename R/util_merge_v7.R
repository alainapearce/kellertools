#' util_merge_v7: Merge all Visit 7 raw Qualtrics databases and organize variable in database order
#'
...
#' v7dat_scored <- util_merge_v7(date_str = '2021-10-11')
#'
#' #if in same working directory as data and covid collecte data has different dates:
#' v7dat_scored <- util_merge_v7(child_date_str = '2021-10-11', child_home_date_str = '2021-9-15', child_lab_date_str = = '2021-9-15', parent_date_str = '2021-10-11')
#'
#' \dontrun{
#' #date must be a string. The following will not run:
#' v7dat_scored <- util_merge_v7(2021-10-11)
#'
#' #date must match the file name - for file named 'Child_V7_Home_2021_09_15', the
#' following will not run:
#' v7dat_scored <- util_merge_v7('2021_10_11')
#' }
#'
#' @seealso Raw data from Qualtrics is processed using the following scripts: \code{\link{util_child_v7dat}}, \code{\link{util_child_v7dat_home}}, \code{\link{util_child_v7dat_lab}}, \code{\link{util_parent_v7dat}}. Visit 7 data is scored using the following scripts: \code{\link{score_ctc}}, \code{\link{score_audit}}
#'
#'
#' @export

util_merge_v7 <- function(date_str, child_date_str, child_home_date_str, child_lab_date_str, parent_date_str, data_path) {
    ...
    if (isTRUE(datestr_arg)) {
        child_v7dat <- util_child_v7dat(date_str, data_path)
        child_home_v7dat <- util_child_v7dat_home(date_str, data_path)
        child_lab_v7dat <- util_child_v7dat_lab(date_str, data_path)
        parent_v7dat <- util_parent_v7dat(date_str, data_path)
        parent_home_v7dat <- util_parent_v7dat_home(date_str, data_path)
    } else {
        child_v7dat <- util_child_v7dat(child_date_str, data_path)
        child_home_v7dat <- util_child_v7dat_home(child_home_date_str, data_path)
        child_lab_v7dat <- util_child_v7dat_lab(child_lab_date_str, data_path)
        parent_v7dat <- util_parent_v7dat(parent_date_str, data_path)
        parent_home_v7dat <- util_parent_v7dat(parent_home_date_str, data_path)
    }
