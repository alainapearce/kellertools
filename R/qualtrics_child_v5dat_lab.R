...
qualtrics_child_v2dat_lab <- function(date_str, data_path) {
    ...
    if (isTRUE(datapath_arg)) {
        qv5_child_path <- paste0(data_path, "/Final_CovidAtHome/Child_V5_Lab_", date_str,
                                 ".sav")
    } else {
    ...
    # 2) selecting relevant data columns
    qv5_child_clean <- qv5_child_dat[c(1, 18, 22:30, 36:38, 40:62, 63:153, 154, 155:184, 186)]

    ## update labels
    qv5_child_clean_labels <- qv5_child_labels[c(1, 18, 22:30, 36:38, 40:62, 63:153, 154, 155:184, 186)]

    # 3) removing all practice events (e.g., 999)
    qv5_child_clean <- qv5_child_clean[!is.na(qv5_child_clean$ID) &
        qv5_child_clean$ID < 999, ]

    # 4) re-ordering and re-name data columns general order: 1) child
    # information (ID, date), 2) freddies, 3) food VAS 4) intakes (meal,
    # meal duration) 5) LOC, interoception 6) notes
    ...
    ## Manually rename
    names(qv5_child_clean)[1:41] <- c("id", "start_date", "freddy_pre_meal",
