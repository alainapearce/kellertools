#' util_fbs_parent_v1dat: Process raw Qualtrics visit 1 data for the parent
#'
...
#'
#' The databases MUST follow the naming convention: Parent_V1_YYYY-MM-DD.sav
#'
#' @param file_pattern string with the pattern to search for to find the raw data file. The pattern must contain respondent and visit number (e.g., for files from parent visit 1, would enter 'Parent_V1').
#' @param data_path (optional) the full path to the directory where the raw Qualtrics database are saved, EXCLUDING the .sav file name (e.g., '.../b-childfoodlab_Shared/Active_Studies/RO1_Brain_Mechanisms_IRB_5357/Participant_Data/untouchedRaw/Qualtrics_Raw/'). If NOT entered, will assume the database is saved in the working directory.
#'
#' @return A list containing: 1) data: data.frame with raw, cleaned data from parent visit 1 Qualtrics; 2) dict: all variable descriptions; 3) pna_data: data.frame marking participants who 'prefered not to answer' (pna) specific questions; and 4) pna_dict: all variable descriptions for pna_data
#'
#' @examples
#' #if in same working directory as data:
#' p_v1_dat <- util_fbs_parent_v1dat('Parent_V1')
#'
#' \dontrun{
...
#' p_v1_dat <- util_fbs_parent_v1dat('Parent')
#' }
#'
#'
#' @export
#'
util_fbs_parent_v1dat <- function(file_pattern, data_path) {
    ...
    if (isTRUE(filepat_arg) & !is.character(file_pattern)) {
        stop("file_pattern must be entered as a string: e.g., 'Parent_V1'")
    } else if (isFALSE(filepat_arg)) {
        stop("file_pattern must set to the a string matching the name of the raw data file for parent visit 1: e.g., 'Parent_V1'")
    }
    ...
    # Qualtrics data
    if (isTRUE(datapath_arg)) {
        qv1_parent_pathlist <- list.files(path = data_path, pattern = file_pattern, full.names = TRUE)
    } else {
        qv1_parent_pathlist <- paste0(pattern = file_pattern, full.names = TRUE)
    }

    # check number of files found
    if (length(qv1_parent_pathlist) > 1) {
    ...
    level99_issue_catvars <- names(qv1_parent_clean)[c(3, 8, 10, 13:14, 89, 92:93, 123:124, 126:127, 129, 131:132, 134:135, 137:140, 142, 144, 147:155, 157, 159:160, 203:212, 226, 232, 238, 244, 250, 256, 262, 268, 274, 280, 286, 292,  298, 304, 310, 316, 322, 328, 334, 340, 346, 352, 358)]
    ...
    qv1_parent_clean <- merge(qv1_parent_clean, visit_dates[c('id', 'RO1_V1_Date')], by = 'id', all.x = TRUE, all.y = FALSE)

    #update start_date
    qv1_parent_clean[["start_date"]] <- ifelse(!is.na(qv1_parent_clean[['RO1_V1_Date']]), as.character(qv1_parent_clean[['RO1_V1_Date']]), as.character(qv1_parent_clean[["start_date"]]))

    #remove RO1_V date column
    qv1_parent_clean <- qv1_parent_clean[, names(qv1_parent_clean) != "RO1_V1_Date"]

    # add label
    qv1_parent_clean_labels[["start_date"]] <- "date from participant contacts databases ('verified_visit_dates*.csv) converted to format yyyy-mm-dd in R. If no date in database, uses start_date metadata from qualtrics"
    ...
    ## sex - make sure always matches across parent/child and visits
    qv1_parent_clean[['parent_respondent']] <- sjlabelled::set_labels(qv1_parent_clean[['parent_respondent']], labels = c(Mother = 0, Father = 1, Other = 2))
    set_attr <- attributes(qv1_parent_clean[["parent_respondent"]])
    qv1_parent_clean[['parent_respondent']] <- ifelse(is.na(qv1_parent_clean[['parent_respondent']]), NA, ifelse(qv1_parent_clean[['parent_respondent']] == 1, 0, ifelse(qv1_parent_clean[['parent_respondent']] == 2, 1, 2)))
    attributes(qv1_parent_clean[['parent_respondent']]) <- set_attr
    qv1_parent_clean_labels[["parent_respondent"]] <- paste0(qv1_parent_clean_labels[["parent_respondent"]], " re-leveled in R to start with 0")
    ...
    qv1_parent_clean <- qv1_parent_clean[c(1:12, 402, 13:28, 380, 29:31, 381, 32, 382, 33:39, 383, 40, 384, 41:43, 385, 44, 386, 45:51, 387, 52, 388, 53:55, 389, 56, 390, 57:63, 391, 64, 392, 65:67, 393, 68, 394, 69:75, 395, 76, 396, 77:79, 397, 80, 398, 81:87, 399, 88, 89:103, 378, 104:106, 379, 403:408, 107:133, 400, 134:136, 401, 137:377)]

    qv1_parent_clean_labels <- qv1_parent_clean_labels[c(1:12, 402, 13:28, 380, 29:31, 381, 32, 382, 33:39, 383, 40, 384, 41:43, 385, 44, 386, 45:51, 387, 52, 388, 53:55, 389, 56, 390, 57:63, 391, 64, 392, 65:67, 393, 68, 394, 69:75, 395, 76, 396, 77:79, 397, 80, 398, 81:87, 399, 88, 89:103, 378, 104:106, 379, 403:408, 107:133, 400, 134:136, 401, 137:377)]
