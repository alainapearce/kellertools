#' util_fbs_child_v3dat: Process raw qualtrics visit 3 data for the child
#'
#' This function loads the .sav raw data file for the child visit 3 data that was collected via Qualtrics and cleans the data. Cleaning the data involves:
...
#'
#' The databases MUST follow the naming convention: Child_V3_YYYY-MM-DD.sav
#'
...
#' child_v3_dat <- util_fbs_child_v3dat('Child_V3')
#'
#' \dontrun{
...
#' child_v3_dat <- util_fbs_child_v3dat('Child')
#' }
#'
#'
#' @export
#'
util_fbs_child_v3dat <- function(file_pattern, data_path) {
    ...
    # Qualtrics data
    if (isTRUE(datapath_arg)) {
        qv3_child_pathlist <- list.files(path = data_path, pattern = file_pattern, full.names = TRUE)
    } else {
        qv3_child_pathlist <- paste0(pattern = file_pattern, full.names = TRUE)
    }

    # check number of files found
    if (length(qv3_child_pathlist) > 1) {
    ...
    # 1) child information (ID. date), 2) freddies, 3) food, VAS 4) intakes (meal, meal duration), 5) DD 6) notes
    ...
    qv3_child_clean <- merge(qv3_child_clean, visit_dates[c('id', 'RO1_V3_Date')], by = 'id', all.x = TRUE, all.y = FALSE)

    #update start_date
    qv3_child_clean[["start_date"]] <- ifelse(!is.na(qv3_child_clean[['RO1_V3_Date']]), as.character(qv3_child_clean[['RO1_V3_Date']]), as.character(qv3_child_clean[["start_date"]]))

    #remove RO1_V date column
    qv3_child_clean <- qv3_child_clean[, names(qv3_child_clean) != "RO1_V3_Date"]

    # add label
    qv3_child_clean_labels[["start_date"]] <- "date from participant contacts databases ('verified_visit_dates*.csv) converted to format yyyy-mm-dd in R. If no date in database, uses start_date metadata from qualtrics"
    ...
    for (var in 1:length(intake_vars)) {
        ...
        qv3_child_clean[[var_name]] <- ifelse(qv3_child_clean[[var_name]] == "-" | qv3_child_clean[[var_name]] == "n/a",
            NA, ifelse(qv3_child_clean[[var_name]] == "312.92*", "312.92", ifelse(qv3_child_clean[[var_name]] == "490.92*",
                "490.92", ifelse(qv3_child_clean[[var_name]] == "69.59*", "69.59", ifelse(qv3_child_clean[[var_name]] ==
                  "667,83", "667.83", qv3_child_clean[[var_name]])))))
