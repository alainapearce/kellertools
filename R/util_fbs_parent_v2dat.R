...
util_fbs_parent_v2dat <- function(file_pattern, data_path) {
    ...
    if (isTRUE(datapath_arg)) {
        qv2_parent_path <- list.files(path = data_path, pattern = file_pattern, full.names = TRUE)
    } else {
        qv2_parent_path <- paste0(pattern = file_pattern, full.names = TRUE)
    }

    # check number of files found
    if (length(qv2_parent_path) > 1) {
    ...
    } else if (length(qv2_parent_path) == 0) {
