...
score_spsrq <- function(spsrq_data, parID) {
    ...
    spsrq_score_dat[["spsrq34_punishment"]] <- rowSums(spsrq_data[sp34_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq34_punishment"]] <- paste0("SPSRQ-34 Sensitivity to Punishment Score")

    # Impulsivity/Fun Seeking
    impfun34_vars <- c("spsrq21", "spsrq25", "spsrq33", "spsrq35", "spsrq37", "spsrq39", "spsrq47")
    spsrq_score_dat[["spsrq34_impfun"]] <- rowSums(spsrq_data[impfun34_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq34_impfun"]] <- paste0("SPSRQ-34 Impulsivity/Fun Seeking Score")

    # Drive
    drive34_vars <- c("spsrq27", "spsrq41", "spsrq43", "spsrq45")
    spsrq_score_dat[["spsrq34_drive"]] <- rowSums(spsrq_data[drive34_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq34_drive"]] <- paste0("SPSRQ-34 Drive Score")

    # Reward Responsiveness
    reward34_vars <- c("spsrq2", "spsrq7", "spsrq9", "spsrq15", "spsrq17", "spsrq19", "spsrq23")
    spsrq_score_dat[["spsrq34_rewardresp"]] <- rowSums(spsrq_data[reward34_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq34_rewardresp"]] <- paste0("SPSRQ-34 Reward Responsiveness Score")

    # Fear/Shyness
    fearshy48_vars <- c("spsrq14", "spsrq16", "spsrq18", "spsrq24", "spsrq26", "spsrq30", "spsrq32", "spsrq34", "spsrq40")
    spsrq_score_dat[["spsrq48_fearshy"]] <- rowSums(spsrq_data[fearshy48_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq48_fearshy"]] <- paste0("SPSRQ-48 Fear/Shyness Score")

    # Anxiety
    anxiety48_vars <- c("spsrq8", "spsrq10", "spsrq20", "spsrq46", "spsrq48")
    spsrq_score_dat[["spsrq48_anxiety"]] <- rowSums(spsrq_data[anxiety48_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq48_anxiety"]] <- paste0("SPSRQ-48 Anxiety Score")

    # Conflict Avoidance
    conflict48_vars <- c("spsrq6", "spsrq22")
    spsrq_score_dat[["spsrq48_conflictavoid"]] <- rowSums(spsrq_data[conflict48_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq48_conflictavoid"]] <- paste0("SPSRQ-48 Conflict Avoidence Score")

    # Impulsivity/Fun Seeking
    impfun48_vars <- c("spsrq21", "spsrq23", "spsrq25", "spsrq35", "spsrq37", "spsrq39")
    spsrq_score_dat[["spsrq48_impfun"]] <- rowSums(spsrq_data[impfun48_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq48_impfun"]] <- paste0("SPSRQ-48 Impulsivity/Fun Seeking Score")

    # Drive
    drive48_vars <- c("spsrq27", "spsrq41", "spsrq43", "spsrq45", "spsrq47")
    spsrq_score_dat[["spsrq48_drive"]] <- rowSums(spsrq_data[drive48_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq48_drive"]] <- paste0("SPSRQ-48 Drive Score")

    # Responsiveness to Social Approval
    social48_vars <- c("spsrq7", "spsrq11", "spsrq13", "spsrq19")
    spsrq_score_dat[["spsrq48_socialapproval"]] <- rowSums(spsrq_data[social48_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq48_socialapproval"]] <- paste0("SPSRQ-48 Responsiveness to Social Approval Score")

    # Sensory Reward
    reward48_vars <- c("spsrq29", "spsrq31")
    spsrq_score_dat[["spsrq48_sensoryreward"]] <- rowSums(spsrq_data[reward48_vars])

    ## add labels to data
    spsrq_score_dat_labels[["spsrq48_sensoryreward"]] <- paste0("SPSRQ-48 Sensory Reward Score")
